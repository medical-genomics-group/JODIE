...
```
pip install numpy loguru scikit-allel zarr==2.17.2 pandas tqdm
```
...
def main(inputfiles, index_trios, index_duos, only_trios, dir, k):
    ...
    for l in inputfiles:
        ...
        #logger.info(f"{callset['variants/REF']=}")
        #logger.info(f"{callset['variants/ALT'][:,0]=}")
        ...
        logger.info(f"{np.unique(xa, return_counts = True)=}")
        #logger.info(f"{np.argwhere(np.equal(xa,18).all(axis=1))=}")
        ...
        x[(k-1)::k] = xpoo
        logger.info(f"{x=}")
        logger.info(f"{np.argwhere(np.equal(x,9).all(axis=1))=}")
        ...
        if 9 in np.unique(x):
            logger.info(f"CHECK DATASET FOR NANS!")
        #save rsids
