...
```
pip install numpy loguru scipy tqdm mpi4py welford matplotlib zarr dask
```
run with n processes:
put in either g or gindex
mpiexec -n 4 python -m mpi4py parental_gibbs_stdX_MPI.py --n 10000 --p 20000 --g 10000 10000 --iters 500 --burnin 100 --x MC/genotype.zarr --y MC/phenotype.txt --resultdir results
...
def sample_V(beta, L, D, k, Z, a, b, s):
    ...
    if Z==0 or np.sum(np.diag(beta_2)) < 0.001:
        V = np.zeros((k,k))
...
def main(n, p, k, groups, gindex, iters, burnin, xfiles, xtxfiles, yfile, itc, resultdir, rmid):
    ...
    if p_split*worldSize-p > 0:
        az = np.zeros((xdata.shape[0], (p_split*worldSize-p)*k))
    ...
    for it in trange(iters, desc="Main loop"):
        ...
        if rank == 0:
            ...
            if it >= burnin:
                ...
                if it == burnin:
                    dfm = pd.DataFrame(beta, columns=['child', 'mother', 'father', 'poo'])
                    ...
                    np.savetxt(resultdir+'/trace_Z_'+str(it)+'.txt', trace_Z)
                    np.savetxt(resultdir+'/trace_sigma2_'+str(it)+'.txt', trace_sigma2)

            if (it%1000==0) and (it > burnin):
                logger.info(f"{it=}")
