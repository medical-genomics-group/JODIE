...
def main(n, p, k, groups, iters, burnin, xfile, yfile, itc, resultdir, inputdir):
    ...
    if rank == 0:
        ...
        if p_split*worldSize-p > 0:
            az = np.zeros((n, k*(p_split*worldSize-p)))
            logger.info(f"Added {p_split*worldSize-p}*{k} columns of zeros to x.")
            xdata = np.concatenate([xdata, az], axis=1)
        xdata = xdata.flatten('F')

        # X0 = np.load(xfile)
        # X = np.array(X0.f.arr_0, order="F") 
        # XtX = np.zeros((k*p,k))
        # for j in range(p):
    ...
    X = np.zeros(k*p_split*n, order="F")
    comm.Scatterv(sendbuf=[xdata, k*p_split*n, MPI.DOUBLE], recvbuf=X, root=0)
    X = X.reshape((n,p_split*k), order='F')
    if rank == 0:
        del xdata
    XtX = np.zeros((k*p,k))
    for j in range(p_split):
        XtX[j*k:k*(j+1),:] = np.matmul(X[:,j*k:k*(j+1)].T, X[:,j*k:k*(j+1)])
    ...
    for it in trange(iters, desc="Main loop"):
        ...
        for j in rj:
            ...
            if gj >= p:
                ...
                xe = blas.dgemv(1.0, X[:,j*k:k*(j+1)], epsilon, trans=1)
                ...
                diff += blas.dgemv(1, X[:,j*k:k*(j+1)], (prev_beta - beta_split[j]))
