...
```
python get_mean_std_rmids.py --indir indir/ --outdir outputdir/ --rmid rmid.txt --pheno pheno
```
--indir directory where genotype files are stored without chr (required)
...
def main(indir, outdir, nchr, k, rmid, pheno):
    ...
    zm = zarr.array(mean)
    logger.info(f"{zm=}")
    zarr.save(outdir+"/mean.zarr", zm)
    zs = zarr.array(std)
    logger.info(f"{zs=}")
    zarr.save(outdir+"/std.zarr", zs)
